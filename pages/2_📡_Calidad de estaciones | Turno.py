...
@st.cache_resource
def libr_cal():
    create_doc = st.session_state["create_doc"]
    plot_CalEst = st.session_state["plot_CalEst"] 
    
    return create_doc, plot_CalEst

create_doc, plot_CalEst = libr_cal()
...
def est1_fechas(red,est_,sens_):
    ...
    all_est = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/{red}/Tablas_Cal_tiempos/*")
    #print(all_est)
    estaciones = []
    for est in all_est:
    ...
    with col2:
        list_est = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/{red}/Tablas_Cal_tiempos/{option}*")
        ...
        for e in list_est:
            ...
            if option != "ROSC":   
                ...
                nom_sensores = e.split("/")[-1].split("_")[1][0:2]
                if nom_sensores not in sensores:
                    
                    #sensores.append(e[-14:-12])
                    sensores.append(nom_sensores)
        ...
        list_est_sen = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/{red}/Tablas_Cal_tiempos/{option}_{option_sen}Z*")
    ...
    if red == "CM" or red == "INT":
        ...
        with st.expander("Estado Actual"):
            ...
            with col2:
                ...
                f_prob = datetime.now()
                ##Comentario anterior
                file_esta = os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Estados/{option}.csv"
                ...
                if st.button("Actualizar estado", key="but_create1"):
                    plot_CalEst.actualizar_estado(option,option_sen,act_sis,opt_e_disp,opt_p_sist,f_prob,opt_f_act,act_elec) 
                    st.rerun() 
    ...
    with tabz:
        ...
        if option_cod_sen == ".":
            list_est = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/{red}/Tablas_Cal_tiempos/{option}_{option_sen}{com}*")
        if len(option_cod_sen) > 1:
            list_est = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/{red}/Tablas_Cal_tiempos/{option}_{option_sen}{com}_{option_cod_sen}*")
...
def est1_informe_s():
    ...
    folder_inf= os.path.dirname(os.path.abspath(__file__))+'/Report_CalEst/Inf_semestrales/'
    #Seleccion de Estacion
    all_est = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/CM/Tablas_Cal_tiempos/*")
    estaciones = []
    for est in all_est:
        nom_est = est.split("/")[-1].split("_")[0]
        if nom_est not in estaciones:
    ...
    with col2:
        list_est = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/CM/Tablas_Cal_tiempos/{option_est}*")
        ...
        list_est_sen = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/CM/Tablas_Cal_tiempos/{option_est}_{option_sen}Z*")
        cod_sensor=[]
        option_cod_sen="."
        if len(list_est_sen) == 2:
            for s in list_est_sen:
    ...
    semestre = ["2023-II","2023-I","2022-II"]
    ...
    for all_sen in sensores:
        ...
        #plot_CalEst.create_info_s_all(option_est,all_sen,fecha_ini, fecha_fin,folder_inf,option_s)
    ...
    with st.expander("Estado Actual"):
        col1, col2 =st.columns(2)
        with col1:
            plot_CalEst.estado_a(option_est,option_sen)
        with col2:
            
            opt_f_act = datetime.now()
            f_prob = datetime.now()
            ##Comentario anterior
            file_esta = os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Estados/{option_est}.csv"
            if os.path.exists(file_esta) == True:        
            ...
            if st.button("Actualizar estado", key="but_create2"):
                plot_CalEst.actualizar_estado(option_est,option_sen,act_sis,opt_e_disp,opt_p_sist,f_prob,opt_f_act,act_elec) 
                st.rerun() 
    ...
    if option_cod_sen == ".":
        ...
        file_z = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/CM/Tablas_Cal_tiempos/{option_est}_{option_sen}{z}*")
        plt_inf_z = plot_CalEst.Plot(option_est,option_sen,fecha_ini,fecha_fin,file_z) 


        file_n = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/CM/Tablas_Cal_tiempos/{option_est}_{option_sen}{n}*")
        if len(file_n) != 0:
            plt_inf_n = plot_CalEst.Plot(option_est,option_sen,fecha_ini,fecha_fin,file_n) 

        file_e = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/CM/Tablas_Cal_tiempos/{option_est}_{option_sen}{e}*")
        if len(file_e) != 0:
            plt_inf_e = plot_CalEst.Plot(option_est,option_sen,fecha_ini,fecha_fin,file_e)    
            
    if len(option_cod_sen) > 1:
        
        file_z = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/CM/Tablas_Cal_tiempos/{option_est}_{option_sen}{z}_{option_cod_sen}*")
        plt_inf_z = plot_CalEst.Plot(option_est,option_sen,fecha_ini,fecha_fin,file_z) 


        file_n = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/CM/Tablas_Cal_tiempos/{option_est}_{option_sen}{n}_{option_cod_sen}*")
        if len(file_n) != 0:
            plt_inf_n = plot_CalEst.Plot(option_est,option_sen,fecha_ini,fecha_fin,file_n) 

        file_e = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/CM/Tablas_Cal_tiempos/{option_est}_{option_sen}{e}_{option_cod_sen}*")
        if len(file_e) != 0:
            plt_inf_e = plot_CalEst.Plot(option_est,option_sen,fecha_ini,fecha_fin,file_e)  
    ...
    with tab_fun:
        ...
        with col2:
            ...
            folder_semestre = os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Inf_semestrales/{option_est}_{option_sen}/{option_s}/"
    ...
    with tab_cal:
        ...
        with col1:
            ...
            img_ruido = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Inf_semestrales/{option_est}_{option_sen}/{option_s}/img_ruido{option_cod_sen}_*")
        ...
        with col2:
            ...
            if st.button("Guardar", help="Ingrese los comentarios y luego de Guardar, al volver a guardar se reemplazarán siempre los comentarios", key="but_cal"):
                ...
                if len(image_ruido) ==0:
                    if option_cod_sen == ".": option_cod_sen=""
                    os.system(f"rm {os.path.dirname(os.path.abspath(__file__))}/Report_CalEst/Inf_semestrales/{option_est}_{option_sen}/{option_s}/img_ruido{option_cod_sen}_*")
                    if os.path.exists(folder_semestre + f"inf_{option_est}_{option_sen}{option_cod_sen}.json") == True:
                        with open(folder_semestre + f"inf_{option_est}_{option_sen}{option_cod_sen}.json", 'r') as file:
                            inf_json_cal = json.load(file)
    ...
    with tab_rec:
        ...
        if st.button("Guardar", help="Ingrese los comentarios y luego de Guardar, al volver a guardar se reemplazarán siempre los comentarios", key="but_recom"):
            ...
            if len(image_recom) ==0:
                os.system(f"rm {os.path.dirname(os.path.abspath(__file__))}/Report_CalEst/Inf_semestrales/{option_est}_{option_sen}/{option_s}/img_recom{option_cod_sen}_*")
        ...
        img_recom = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Inf_semestrales/{option_est}_{option_sen}/{option_s}/img_recom{option_cod_sen}_*")
    ...
    with col1:
        if est_din[0][0] == 0 and est_din[0][1] == 0 and est_din[0][2] == 0:
            ...
            if st.button("Crear Documento", key="but_create3"):
                ...
                #create_doc.mk_doc_all(option_est,option_est,cod_l,option_s)
                #descargar
                folder_semestre = os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Inf_semestrales/{option_est}_{option_sen}/{option_s}/"
    with col2:
...
def est1_informe_m():
    ...
    folder_inf= os.path.dirname(os.path.abspath(__file__))+'/Report_CalEst/Inf_mensuales/'
    #Seleccion de Estacion
    all_est = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Datos/CM/Tablas_Cal_tiempos/*")
    estaciones = []
    for est in all_est:
    ...
    with tab_fun:
        ...
        with col2:
            ...
            folder_mes = os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Inf_mensuales/{option_est}_{option_sen}/{option_year}/{option_month}/"
    ...
    with tab_cal:
        ...
        with col1:
            ...
            img_ruido = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Inf_mensuales/{option_est}_{option_sen}/{option_year}/{option_month}/img_ruido{option_cod_sen}_*")
            for image_file in img_ruido:
                #file_details_damage = {"filename":image_file.name,"filetype":image_file.type,
                #                "filesize":image_file.size}
                #st.write(file_details_damage)
        ...
        with col2:
            ...
            if st.button("Guardar", help="Ingrese los comentarios y luego de Guardar, al volver a guardar se reemplazarán siempre los comentarios", key="but_cal"):
                ...
                if len(image_ruido) ==0:
                    if option_cod_sen == ".": option_cod_sen=""
                    os.system(f"rm {os.path.dirname(os.path.abspath(__file__))}/Report_CalEst/Inf_semestrales/{option_est}_{option_sen}/{option_year}/{option_month}/img_ruido{option_cod_sen}_*")
                    if os.path.exists(folder_mes + f"inf_{option_est}_{option_sen}{option_cod_sen}.json") == True:
                        with open(folder_mes + f"inf_{option_est}_{option_sen}{option_cod_sen}.json", 'r') as file:
                            inf_json_cal = json.load(file)
    ...
    with tab_rec:
        ...
        if st.button("Guardar", help="Ingrese los comentarios y luego de Guardar, al volver a guardar se reemplazarán siempre los comentarios", key="but_recom"):
            ...
            if len(image_recom) ==0:
                ...
                os.system(f"rm {os.path.dirname(os.path.abspath(__file__))}/Report_CalEst/Inf_mensuales/{option_est}_{option_sen}/{option_year}/{option_month}/img_recom{option_cod_sen}_*")
        ...
        img_recom = glob.glob(os.path.dirname(os.path.abspath(__file__))+f"/Report_CalEst/Inf_mensuales/{option_est}_{option_sen}/{option_year}/{option_month}/img_recom{option_cod_sen}_*")
        for image_file in img_recom:
            #file_details_damage = {"filename":image_file.name,"filetype":image_file.type,
            #                "filesize":image_file.size}
            #st.write(file_details_damage)
    ...
    with col1:
        if est_din[0][0] == 0 and est_din[0][1] == 0 and est_din[0][2] == 0:
            ...
            if st.button("Crear Documento", key="but_create4"):
...
def estado_est():
    ...
    st.title("**Estado Actual de las estaciones**")
    st.write("_______")
    st.write(".")
...
def main():
    ...
    image = Image.open(os.path.dirname(os.path.abspath(__file__))+'/Report_CalEst/images/Simbolo_SGC_Blanco.png')
    st.sidebar.image(image)
    st.sidebar.title("""Reporte Calidad Estacion""")
    st.sidebar.write("_______________________________________________")
    folder = os.path.dirname(os.path.abspath(__file__))+'/Report_CalEst/informes_estaciones'
    

    #Tipo de Consulta
    option_consulta = st.sidebar.selectbox('**Consulta**',["Estado Actual","Por fechas","Informe semestral","Informe mensual"] ,key ="cons")
    ...
    with st.sidebar.expander("**Sobre la app Report_CalEst**"):
        st.write("""
                **Calidad de estaciones (Reporte de funcionamiento y calidad de estaciones)**\n 
                """)
        st.write(""" 
